...
class TradeRoutePreviewWidget(QWidget):
    def __init__(self, parent: QWidget | None = None) -> None:
        super().__init__(parent)
        self.preview_data: TradeRoutePreviewData | None = None
    ...
    def set_preview_data(self, preview_data: TradeRoutePreviewData | None) -> None:
        self.preview_data = preview_data
        self._render_preview()

    def _render_preview(self) -> None:
        self.scene.clear()
        if self.preview_data is None or not self.preview_data.systems:
            placeholder = self.scene.addText("No preview available")
            placeholder.setDefaultTextColor(QColor("#94a3b8"))
            placeholder.setPos(24, 20)
            return
